...
class TweetViewModel: ObservableObject {
    ...
    var sensitiveTweets: [Tweet] {
        tweets.filter { $0.possibly_sensitive }
    }
    
    var toxicTweets: [Tweet] {
        tweets.filter { tweet in
            toxicityResults[tweet.id]?.isToxic == true
        }
    }
    ...
    func classifyAllTweets() async {
        ...
        print("✅ Classified \(results.count) tweets. Found \(toxicTweets.count) toxic tweets.")
    }
} */
