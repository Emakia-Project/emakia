"""
Consensus Analysis for CoreML and LLM Predictions (95% Threshold Version)

This version applies a stricter threshold: CoreML predictions are only considered
toxic (0) if the coreml_score is >= 0.95. Otherwise treated as neutral (1).
...
OUTPUT_DIR = "consensus_analysis_95threshold"
COREML_TOXIC_THRESHOLD = 0.95  # Score must be >= 0.95 to be considered toxic

# Output files
UNIQUE_CONTENT_FILE = os.path.join(OUTPUT_DIR, "coreml_llm_predictions_uniquecontent_95threshold.csv")

# Perfect agreement files
AGREE_TOXIC_FILE = os.path.join(OUTPUT_DIR, "perfect_agreement_toxic_95threshold.csv")
AGREE_NEUTRAL_FILE = os.path.join(OUTPUT_DIR, "perfect_agreement_neutral_95threshold.csv")

# Disagreement files
DISAGREE_TOXIC_FILE = os.path.join(OUTPUT_DIR, "disagreement_toxic_95threshold.csv")
DISAGREE_NEUTRAL_FILE = os.path.join(OUTPUT_DIR, "disagreement_neutral_95threshold.csv")

# Analysis reports
SUMMARY_REPORT = os.path.join(OUTPUT_DIR, "consensus_summary_report_95threshold.txt")
MAJORITY_VOTE_FILE = os.path.join(OUTPUT_DIR, "majority_vote_results_95threshold.csv")
...
def normalize_coreml_prediction(pred_value, score_value):
    """
    Normalize CoreML prediction to 0 (toxic) or 1 (neutral)
    APPLIES 95% THRESHOLD: Only toxic if score >= 0.95
    
    Args:
        pred_value: Original CoreML prediction
        score_value: CoreML confidence score
    
    Returns:
        0 (toxic) if score >= 0.95, otherwise 1 (neutral)
    """
    ...
    if isinstance(pred_value, str):
        ...
        if "neutral" in pred_lower:
            original_pred = 1
        elif "harassment" in pred_lower or "toxic" in pred_lower:
            original_pred = 0
        else:
            # Try to convert string numbers
            try:
                original_pred = int(pred_value)
    ...
    # If original prediction is neutral, keep it neutral
    if original_pred == 1:
        return 1
    
    # If original prediction is toxic, check the score threshold
    if original_pred == 0:
        try:
            score = float(score_value)
            # Only consider toxic if score >= 0.95
            if score >= COREML_TOXIC_THRESHOLD:
                return 0  # Toxic
            else:
                return 1  # Reclassified as neutral (score too low)
        except (ValueError, TypeError):
            # If we can't parse the score, keep original prediction
            return original_pred
    
    return None


def normalize_possibly_sensitive(sensitive_value):
    """Normalize possibly_sensitive to 0 or 1"""
...
def get_all_predictions(row):
    """Extract all model predictions including CoreML (with 95% threshold)"""
    predictions = {}
    
    # CoreML prediction with 95% threshold
    coreml_pred = normalize_coreml_prediction(
        row.get("coreml_prediction"), 
        row.get("coreml_score")
    )
    predictions["coreml"] = coreml_pred
...
def analyze_dataset(rows, dataset_name="Full Dataset"):
    ...
    """
    print("\n" + "="*60)
    print(f"🔍 ANALYZING: {dataset_name} (CoreML Threshold: {COREML_TOXIC_THRESHOLD})")
    print("="*60)
    ...
    stats = {
        ...
        "disagree_toxic": 0,
        "disagree_neutral": 0,
        "reclassified_to_neutral": 0,  # Track how many were reclassified
    ...
    for row in rows:
        # Get original and adjusted CoreML prediction
        original_pred_str = row.get("coreml_prediction", "")
        score = row.get("coreml_score", 0)
        
        # Normalize CoreML prediction with 95% threshold
        coreml_pred = normalize_coreml_prediction(original_pred_str, score)
        
        if coreml_pred is None:
            continue
        
        # Track reclassifications
        original_pred_lower = str(original_pred_str).lower()
        if ("harassment" in original_pred_lower or "toxic" in original_pred_lower or original_pred_str == "0"):
            if coreml_pred == 1:  # Was toxic, now neutral
                stats["reclassified_to_neutral"] += 1
        
        # Normalize possibly_sensitive
        possibly_sensitive = normalize_possibly_sensitive(row.get("possibly_sensitive", 0))
        ...
        majority_row["adjusted_coreml_prediction"] = "Neutral" if coreml_pred == 1 else "Harassment"
    ...
    write_csv(agree_toxic, os.path.join(OUTPUT_DIR, f"{prefix}perfect_agreement_toxic_95threshold.csv"))
    write_csv(agree_neutral, os.path.join(OUTPUT_DIR, f"{prefix}perfect_agreement_neutral_95threshold.csv"))
    write_csv(disagree_toxic, os.path.join(OUTPUT_DIR, f"{prefix}disagreement_toxic_95threshold.csv"))
    write_csv(disagree_neutral, os.path.join(OUTPUT_DIR, f"{prefix}disagreement_neutral_95threshold.csv"))
    write_csv(majority_results, os.path.join(OUTPUT_DIR, f"{prefix}majority_vote_results_95threshold.csv"))
    
    # Print statistics
    print(f"\n📊 {dataset_name} Statistics:")
    print(f"   Total rows analyzed: {stats['total_rows']}")
    print(f"   ⚠️  Reclassified from toxic to neutral (score < {COREML_TOXIC_THRESHOLD}): {stats['reclassified_to_neutral']}")
    print(f"\n   Perfect Agreement:")
...
def generate_summary_report(full_stats, unique_stats):
    ...
    with open(SUMMARY_REPORT, 'w', encoding='utf-8') as f:
        f.write("="*60 + "\n")
        f.write("CONSENSUS ANALYSIS SUMMARY REPORT (95% Threshold)\n")
        f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"CoreML Toxic Threshold: Score >= {COREML_TOXIC_THRESHOLD}\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"Reclassified to neutral (score < {COREML_TOXIC_THRESHOLD}): {full_stats['reclassified_to_neutral']}\n")
        ...
        f.write(f"Reclassified to neutral (score < {COREML_TOXIC_THRESHOLD}): {unique_stats['reclassified_to_neutral']}\n")
        ...
        if total_full > 0:
            ...
            reclass_pct = (full_stats['reclassified_to_neutral'] / total_full) * 100
            f.write(f"Full dataset reclassification rate: {reclass_pct:.1f}%\n")
        ...
        if total_unique > 0:
            ...
            reclass_pct_unique = (unique_stats['reclassified_to_neutral'] / total_unique) * 100
            f.write(f"Unique dataset reclassification rate: {reclass_pct_unique:.1f}%\n")
...
def main():
    ...
    print(f"   CoreML Toxic Threshold: Score >= {COREML_TOXIC_THRESHOLD}")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("✅ CONSENSUS ANALYSIS COMPLETE (95% Threshold)")
    print("="*60)
    ...
    print(f"\nNote: CoreML predictions only considered toxic if score >= {COREML_TOXIC_THRESHOLD}")
    print("="*60)
